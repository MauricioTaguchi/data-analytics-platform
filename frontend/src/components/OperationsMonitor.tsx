...
type Props = { jobs: TrackedJob[]; live: boolean; apiStatus: string; onCancel: (taskId: string) => void };

export function OperationsMonitor({ jobs, live, apiStatus, onCancel }: Props) {
  return <section className="page-grid"><article className="panel"><div className="panel-header"><div><h2>Runtime monitoring</h2><p>Worker progress, failures, and cancellation controls</p></div><span className={live ? "status-pill success" : "status-pill"}>{live ? "Authenticated" : "Demo"}</span></div>
    ...
    <div className="job-list">{jobs.length ? jobs.map((job) => <div className="job-row" key={`${job.task_id}-${job.label}`}><div><strong>{job.label}</strong><span>{job.task_id}</span></div><div className="job-progress"><div><span style={{ width: `${job.progress}%` }} /></div><small>{job.status} · {job.progress}%</small></div>{!new Set(["SUCCESS", "FAILURE", "REVOKED"]).has(job.status) ? <button className="secondary" onClick={() => onCancel(job.task_id)}>Cancel</button> : null}</div>) : <div className="empty-state"><strong>No jobs yet</strong><span>Upload or transform a dataset to populate this monitor.</span></div>}</div>
  </article></section>;
}
